...
def train(num_epochs, model, train_data_loader, val_data_loader, criterion, optimizer, saved_dir, val_every, device,
          model_name):
    # start training
...
def main(args):
    ...
    for fold, (train_index, valid_index) in enumerate(kfold.split(dataset), 1):
        train_sub = torch.utils.data.Subset(dataset, train_index)
        valid_sub = torch.utils.data.Subset(dataset, valid_index)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_dir', type=str, default='./results',
                        help='directory where model would be saved (default: ./results)')

    parser.add_argument('--model', type=str, default='Resnet50',
                        help='backbone bert model for training (default: FCN8s)')
    parser.add_argument('--augmentation', type=str, default='BasicAugmentation',
